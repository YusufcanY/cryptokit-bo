  ...
  const rows = useMemo(() => {
    ...
    if (assetFilter !== 'all') {
      filtered = filtered.filter(
        (tx) => tx.asset.toLowerCase() === assetFilter.toLowerCase(),
      )
    }

    return filtered.map((tx) => {
      let usdValue = 0
      if (prices && prices[tx.asset as keyof CryptoPrices]) {
        usdValue = tx.amount * prices[tx.asset as keyof CryptoPrices].usd
      }
      return {
        ...tx,
        usdValue,
      ...
      <CardContent sx={{ p: { xs: 2, md: 3 } }}>
        ...
        {isTxError && (
          <Alert severity="error" sx={{ mb: 2 }}>
            ...
            {txError?.message || 'Something went wrong'}
          </Alert>
        )}
